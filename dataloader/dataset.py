...
class Load_Dataset(Dataset):
    ...
    def __init__(self, dataset, config):
        ...
        if len(X_train.shape) < 3:
            X_train = X_train.unsqueeze(2) # (N, T, d)
...
def data_generator1(train_data, test_data, train_labels, test_labels, configs):
    ...
    train_x = subsequences(train_time_series, configs.window_size, configs.time_step) # (B , T)
    test_x = subsequences(test_time_series, configs.window_size, configs.time_step) # (B, T)
    ...
    train_dat_dict = dict()
    train_dat_dict["samples"] = train_x  # (B, d, T)
    train_dat_dict["labels"] = train_y  # (B)
