...
class UCR(TSADBaseDataset):
    ...
    def __init__(self, rootdir=None):
        ...
        self.time_series = sorted(
            glob.glob(
                os.path.join(
                    rootdir,  "UCR_Anomaly_FullData", "*.txt"   # AnomalyDatasets_2021
                )
            )
        )
...
class UCR_sub(TSADBaseDataset):
    ...
    def __init__(self, rootdir=None, family=None):
        super().__init__()
        if rootdir is None:
            fdir = os.path.dirname(os.path.abspath(__file__))
            merlion_root = os.path.abspath(os.path.join(fdir, "..", "..", ".."))
        ...
        self.time_series = sorted(
            glob.glob(
                os.path.join(
                    rootdir,  "UCR_Anomaly_ClassData", family ,"*.txt"   # AnomalyDatasets_2021
                )
            )
        )
